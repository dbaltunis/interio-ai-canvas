...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const fetchResponse = await fetch(
      `${Deno.env.get('SUPABASE_URL')}/rest/v1/appointment_notifications?status=eq.pending&scheduled_for=lte.${now}&select=*,appointments(title,start_time,location,video_meeting_link),user_profiles(display_name)`,
      {
        headers: {
    ...
    if (!fetchResponse.ok) {
      throw new Error(`Failed to fetch notifications: ${fetchResponse.statusText}`);
    }
