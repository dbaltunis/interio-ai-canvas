-- Secure user presence and provide sanitized access via RPC (fixed EXECUTE strings)
...
BEGIN
  IF to_regclass('public.user_presence') IS NOT NULL THEN
    ...
    EXECUTE 'CREATE POLICY "Team can view presence" ON public.user_presence FOR SELECT USING (public.get_account_owner(auth.uid()) = public.get_account_owner(user_id))';

    -- Allow users to manage their own presence; admins can manage all via is_admin()
    EXECUTE 'CREATE POLICY "Users manage own presence" ON public.user_presence FOR ALL USING (auth.uid() = user_id OR public.is_admin()) WITH CHECK (auth.uid() = user_id OR public.is_admin())';
  END IF;
END$$;
