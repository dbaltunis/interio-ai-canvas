-- Update get_team_presence function to include status_message
CREATE OR REPLACE FUNCTION public.get_team_presence(search_param text DEFAULT NULL::text)
 RETURNS TABLE(user_id uuid, display_name text, role text, last_seen timestamp with time zone, is_online boolean, status text, status_message text)
 LANGUAGE sql
