-- ============================================================================
-- PHASE 1 & 2: Database Schema Fixes & Permission System Repair
-- ============================================================================
...
-- 5. Drop and recreate accept_user_invitation function
DROP FUNCTION IF EXISTS public.accept_user_invitation(text, uuid);

CREATE FUNCTION public.accept_user_invitation(
...
-- 6. Drop and recreate fix_user_permissions_for_role
DROP FUNCTION IF EXISTS public.fix_user_permissions_for_role(uuid);

CREATE FUNCTION public.fix_user_permissions_for_role(target_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- 7. Drop and recreate handle_new_user_profile trigger function
...
-- 9. Drop and recreate get_team_presence
DROP FUNCTION IF EXISTS public.get_team_presence(text);

CREATE FUNCTION public.get_team_presence(search_param text DEFAULT NULL)
RETURNS TABLE(
  user_id uuid,
  display_name text,
  role text,
