...
-- Fix the seed_account_options function to use order_index instead of sort_order
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      ...
      -- Insert with order_index instead of sort_order
      INSERT INTO treatment_options (
        ...
        default_option.sort_order -- Read from sort_order in source, write to order_index in destination
      )
...
COMMENT ON FUNCTION seed_account_options(UUID) IS 
  'Seeds system default treatment options for a specific account. Fixed to use order_index instead of sort_order.';
