...
ADD COLUMN parent_account_id UUID REFERENCES public.user_profiles(user_id),
ADD COLUMN invited_by_user_id UUID REFERENCES public.user_profiles(user_id);
...
CREATE TABLE public.access_requests (
  ...
  record_type TEXT NOT NULL CHECK (record_type IN ('job', 'client', 'project')),
...
-- Create trigger for updated_at on account_settings
CREATE TRIGGER update_account_settings_updated_at
BEFORE UPDATE ON public.account_settings
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Create trigger for updated_at on access_requests  
CREATE TRIGGER update_access_requests_updated_at
BEFORE UPDATE ON public.access_requests
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Migrate existing owner account data
...
SELECT 
  ...
  COALESCE(
    (SELECT jsonb_object_agg(integration_type, row_to_json(is_data)) FROM (
  ...
  COALESCE(
    (SELECT measurement_units FROM public.business_settings bs3 WHERE bs3.user_id = up.user_id LIMIT 1),
    '{"length": "mm", "area": "sq_m", "fabric": "m", "currency": "AUD"}'
  )::jsonb as measurement_units
