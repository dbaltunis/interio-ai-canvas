        ...
        <div className="w-full">
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-2/5 flex-shrink-0">
              ...
              {treatmentCategory === 'roller_blinds' || treatmentCategory === 'venetian_blinds' || treatmentCategory === 'roman_blinds' || treatmentCategory === 'cellular_blinds' || treatmentCategory === 'vertical_blinds' || treatmentCategory === 'panel_glide' || treatmentCategory === 'plantation_shutters' || selectedTemplate?.curtain_type === 'roller_blind' || selectedTemplate?.curtain_type === 'roman_blind' || selectedTemplate?.curtain_type === 'venetian_blind' || selectedTemplate?.curtain_type === 'vertical_blind' || selectedTemplate?.curtain_type === 'cellular_blind' || selectedTemplate?.curtain_type === 'panel_glide' || selectedTemplate?.curtain_type === 'plantation_shutter' ? <DynamicBlindVisual windowType={windowType} measurements={measurements} template={selectedTemplate} blindType={treatmentCategory === 'roller_blinds' || selectedTemplate?.curtain_type === 'roller_blind' ? 'roller' : treatmentCategory === 'venetian_blinds' || selectedTemplate?.curtain_type === 'venetian_blind' ? 'venetian' : treatmentCategory === 'vertical_blinds' || selectedTemplate?.curtain_type === 'vertical_blind' ? 'vertical' : treatmentCategory === 'roman_blinds' || selectedTemplate?.curtain_type === 'roman_blind' ? 'roman' : treatmentCategory === 'cellular_blinds' || selectedTemplate?.curtain_type === 'cellular_blind' ? 'cellular' : 'roller'} mountType={measurements.mount_type || 'outside'} chainSide={measurements.chain_side || 'right'} /> : <div className="relative container-level-2 rounded-lg p-8 min-h-[400px] overflow-visible">
              ...
              {curtainType === "pair" ? <>
                  ...
                  <div className={`absolute ${hardwareType === "track" ? "top-4" : "top-16"} left-12 w-12 ${getCurtainBottomPosition()} bg-primary/80 rounded-sm shadow-lg`}>
                    ...
                    {poolingOption === "below_floor" && hasValue(poolingAmount) && <div className="absolute -bottom-4 left-0 w-full h-4 bg-primary/60 rounded-b-lg"></div>}
                  </div>
            ...
            <div className="lg:w-3/5 space-y-4">
                  ...
                  <Card className="border-primary/20 bg-primary/5">
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <RadioGroup value={measurements.mount_type || 'outside'} onValueChange={value => handleInputChange("mount_type", value)} disabled={readOnly} className="flex flex-row space-x-4">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {measurements.mount_type === 'inside' ? 'Blind fits inside the window frame' : 'Blind mounts on the wall or outside the frame'}
                        </p>
                      </div>

                      <div>
                        <Label className="text-sm font-medium mb-2 block">Chain/Control Side</Label>
                        <RadioGroup value={measurements.chain_side || 'right'} onValueChange={value => handleInputChange("chain_side", value)} disabled={readOnly} className="flex flex-row space-x-4">
                          <div className="flex items-center space-x-2">
                      ...
                      {(treatmentType === 'roller_blinds' || treatmentType === 'venetian_blinds' || treatmentType === 'roman_blinds' || treatmentType === 'vertical_blinds' || treatmentType === 'cellular_blinds' || treatmentType === 'panel_glide' || treatmentType === 'plantation_shutters') && <DynamicRollerBlindFields measurements={measurements} onChange={handleInputChange} templateId={selectedTemplate?.id} readOnly={readOnly} />}
                    </CardContent>
                  </Card>
                </>}
              ...
              {treatmentCategory === 'curtains' && <div className="container-level-1 rounded-lg p-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  }} disabled={readOnly} className="grid grid-cols-2 gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    }} placeholder="0.00" readOnly={readOnly} className="h-12 pr-16 text-lg font-bold text-center container-level-2 border-2 border-border focus:border-primary text-card-foreground" />
            ...
            {selectedTemplate && <div className="space-y-3">
                ...
                <div className="bg-white border-2 border-purple-200 rounded-lg p-3 shadow-sm">
                  ...
                  <FabricSelectionSection selectedFabric={selectedFabric || ""} onFabricChange={onFabricChange || (() => {})} readOnly={readOnly} fabricCalculation={fabricCalculation} onMeasurementChange={onMeasurementChange} />
                </div>

                {/* Lining Options - Compact */}
                {selectedTemplate?.lining_types && selectedTemplate.lining_types.length > 0 && <div className="container-level-2 border-2 border-purple-200 rounded-lg p-4 shadow-sm">
                    ...
                    <LiningOptionsSection template={selectedTemplate} selectedLining={selectedLining || "none"} onLiningChange={onLiningChange || (() => {})} readOnly={readOnly} />
                  </div>}
                ...
                <div className="container-level-2 border-2 border-orange-200 rounded-lg p-4 shadow-sm">
                  <div className="flex items-center gap-3 mb-3">
                   ...
                   <HeadingOptionsSection template={selectedTemplate} selectedHeading={selectedHeading || "standard"} onHeadingChange={onHeadingChange || (() => {})} readOnly={readOnly} />
                </div>
              </div>}
          </div>
        </div>
