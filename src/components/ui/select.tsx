...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 pointer-events-auto relative z-10",
      className
    )}
    onPointerDown={(e) => {
      console.log("[SELECT] Trigger clicked", e.target);
    }}
    {...props}
  >
    {children}
...
>(({ className, children, position = "popper", ...props }, ref) => (
  ...
  <SelectPrimitive.Content
    ...
    position={position}
    onPointerDown={(e) => {
      console.log("[SELECT] Content clicked", e.target);
      e.stopPropagation();
    }}
    {...props}
  >
