...
import { getFinderColor } from "@/constants/finderColors";
...
export const CollectionsView = ({ onSelectCollection, selectedVendor: externalVendor }: CollectionsViewProps) => {
  ...
  // Filter by selected brand (from parent sidebar), then by search term
  const filteredCollections = collections
    .filter((c: any) => {
      if (!externalVendor) return true;
      if (externalVendor === "unassigned") return !c.vendor_id;
      return c.vendor_id === externalVendor;
    })
    .filter((c: any) => {
        ...
        {filteredCollections.length === 0 && !searchTerm ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredCollections.map((collection: any) => {
              const colorTag = getFinderColor(collection.color_tag);
              return (
                <Card 
                  key={collection.id} 
                ...
                >
                  <CardHeader className="pb-2">
                    ...
                    {collection.vendor && !externalVendor && (
