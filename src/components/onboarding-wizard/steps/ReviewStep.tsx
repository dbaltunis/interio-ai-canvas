...
const SECTION_LABELS: Record<string, { label: string; icon: string; required: boolean }> = {
  company_info: { label: 'Company Information', icon: '🏢', required: true },
  regional_settings: { label: 'Regional Settings', icon: '🌍', required: true },
  document_sequences: { label: 'Document Sequences', icon: '📄', required: false },
  inventory_data: { label: 'Product Inventory', icon: '📦', required: false },
  pricing_grids: { label: 'Pricing Grids', icon: '📊', required: false },
  window_coverings: { label: 'Window Covering Types', icon: '🪟', required: true },
  manufacturing_settings: { label: 'Manufacturing Settings', icon: '🏭', required: false },
  stock_management: { label: 'Stock Management', icon: '📋', required: false },
  email_templates: { label: 'Email Templates', icon: '✉️', required: false },
  quotation_settings: { label: 'Quotation Settings', icon: '💰', required: false },
  integrations_config: { label: 'Integrations', icon: '🔌', required: false },
  users_permissions: { label: 'Team Members', icon: '👥', required: false },
};

export const ReviewStep = ({ data, completionStatus, onComplete }: ReviewStepProps) => {
  const completedCount = Object.values(completionStatus).filter(Boolean).length;
  const totalSections = Object.keys(SECTION_LABELS).length;
  const requiredSections = Object.entries(SECTION_LABELS)
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Review your configuration before applying settings to your account.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="border rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {data.regional_settings?.currency && (
                <div>
                  <span className="text-muted-foreground">Currency:</span>{' '}
                  <span className="font-medium">{data.regional_settings.currency}</span>
                </div>
              )}
              {data.regional_settings?.measurement_units && (
                <div>
                  <span className="text-muted-foreground">Units:</span>{' '}
                  <span className="font-medium capitalize">{data.regional_settings.measurement_units}</span>
                </div>
              )}
              {data.window_coverings && (
                <div>
                  <span className="text-muted-foreground">Products:</span>{' '}
                  <span className="font-medium">
                    {Object.entries(data.window_coverings)
                      .filter(([k, v]) => v === true && k !== 'pricing_methods')
                      .length} types enabled
                  </span>
                </div>
              )}
            </div>
                ...
                <div>
                  ...
                  <ul className="mt-1 text-amber-700 dark:text-amber-300">
                    {requiredSections
                      ...
                      .map(key => (
                        <li key={key}>• {SECTION_LABELS[key].label}</li>
                      ))}
                  </ul>
                </div>
              </div>
