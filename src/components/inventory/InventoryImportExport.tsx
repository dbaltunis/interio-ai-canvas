    ...
    if (format === 'basic') {
      ...
      csvContent = [
        ...
        ...inventory.map(item => [
          ...
          item.max_length || "",
          `"${item.pricing_method || ""}"`,
          `"${item.specifications ? JSON.stringify(item.specifications).replace(/"/g, '""') : ""}"`,
          `"${item.pricing_grid ? JSON.stringify(item.pricing_grid).replace(/"/g, '""') : ""}"`,
          `"${item.images ? item.images.join(';') : ""}"`,
          `"${item.compatibility_tags ? item.compatibility_tags.join(';') : ""}"`
        ].join(","))
      ].join("\n");
    }
