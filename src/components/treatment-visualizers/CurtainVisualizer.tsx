...
export const CurtainVisualizer = ({
  ...
  const renderMeasurementIndicators = () => {
    ...
    return (
      <>
        ...
        {measurements.drop && (
          <div className={`absolute right-4 ${hardwareType === "track" ? "top-6" : "top-18"} ${hasPooling ? "bottom-8" : "bottom-4"} flex flex-col items-center z-20`}>
            {/* Top arrow */}
  ...
  return (
    <div className={`relative min-h-[400px] bg-gradient-to-b from-sky-50 to-sky-100 border-2 border-border rounded-lg overflow-visible ${className}`}>
      ...
      {/* Treatment info - only show when hideDetails is false */}
      {!hideDetails && (
        <div className="absolute top-2 left-2 bg-background/80 backdrop-blur-sm rounded p-2 text-xs space-y-1 z-10">
