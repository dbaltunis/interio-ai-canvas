...
export const ProjectDetailsTab = ({ project, onUpdate }: ProjectDetailsTabProps) => {
  ...
  const getStatusColor = (statusName: string) => {
    const statusDetails = getStatusDetails(statusName);
    if (!statusDetails) return 'bg-gray-100 text-gray-800 border-gray-200';
              ...
              <div className="mt-2">
                {isEditing ? (
                  ...
                  <Badge className={getStatusColor(formData.status)}>
                    {getStatusDetails(formData.status)?.name || formData.status.replace('_', ' ').toUpperCase()}
                  </Badge>
                )}
              </div>
            </div>
